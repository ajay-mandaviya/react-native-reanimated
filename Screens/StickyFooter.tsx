...
//   console.log("bottomAction", animatedRef?.current);
...
//   console.log("topEdge", topEdge);
  ...
  const animatedBottom = useAnimatedStyle(() => {
    ...
    console.log("y====",y , topEdge + 1 , topEdge ,topEdge-1);
  ...
  return (
    <SafeAreaView>
      ...
      >
        {articleParagraphs.map((ele, index) => {
          return (
            <View key={ele}>
              {index % 3 === 0 && (
                <Image style={styles.image} source={{ uri: getImage(index) }} />
              )}
              <Text style={styles.paragraph}>{ele}</Text>
            </View>
          );
        })}
        ...
        <View>
          ...
          {articleParagraphs.slice(0, 3).map((ele, index) => {
            return (
              <View key={`fea~${index}`} style={{ flexDirection: "row" }}>
                <Image
      ...
      <Animated.View
        ...
        onLayout={(e) => {
          console.log("layout event", e.nativeEvent);
            // ref.current = e.nativeEvent
          setBottomAction(e.nativeEvent.layout);
        }}
        style={[
